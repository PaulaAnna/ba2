%% 	This LaTeX Template is meant for "Second Bachelor´s Thesis"
%% 	written by Frank A Titze in March 2009
%% 	for Hochschule Kufstein Tirol, Austria, Dep. Wirtschaftsinformatik
%% 	www.hsk-edu.at
%% 	@author Frank A Titze (frank.a.titze@gmx.eu)
%% 	@copyright 2009, FAT
%% 	@license GPL
%% 	@version 1.0
%%=================================================
%% 	Main Document  Vorlage_BA2_by.fat.tex
%% 	This document:  appendix.tex
%%=================================================
\pagestyle{fancy} \fancyhf{} \pagenumbering{arabic} \cfoot{A\thepage}




\chapter{Vorstellung und Vorstellung des Themas der Bachelorarbeit}

\includepdf[pages=-]{content/Einleitung_Interview.pdf}

\chapter{Interviewleitfaden}

\includepdf[pages=-]{content/Interviewleitfaden.pdf}

\chapter{Transkribierte Interviews}

\includepdf[pages=-]{content/Alle_Interviews.pdf}

%%EOF@fat
